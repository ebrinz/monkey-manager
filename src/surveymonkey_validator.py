...
class SurveyMonkeyValidator:
    """
    Validates that all files referenced in the Survey Monkey mapping file have corresponding
    JSON output files, and reports any missing or problematic files.
    Specific to Survey Monkey file mappings with respondent_id format.
    """
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python surveymonkey_validator.py <mapping_file> <output_dir1> [<output_dir2> ...]")
        sys.exit(1)
    ...
    validator = SurveyMonkeyValidator(mapping_file, output_dirs)
